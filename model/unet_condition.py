...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    UNet2DConditionModel is a conditional 2D UNet model that takes in a noisy sample, conditional state(text-style annotations)
...
#         self.time_embedding = TimestepEmbedding(timestep_input_dim, time_embed_dim)
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block(
                ...
                add_downsample=not is_final_block, ## No downsampler in the final_block
                resnet_eps=norm_eps,
...
#         t_emb = self.time_proj(timesteps) ## [BS] --> [BS, Dim]

        # timesteps does not contain any weights and will always return f32 tensors
        # but time_embedding might actually be running in fp16. so we need to cast here.
        # there might be better ways to encapsulate this.
#         t_emb = t_emb.to(dtype=self.dtype)
#         emb = self.time_embedding(t_emb) ## [BS, Dim] --> [BS, 1280]
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                ...
                sample = upsample_block(
                    hidden_states=sample, res_hidden_states_tuple=res_samples, upsample_size=upsample_size
                )
        # 6. post-process
