...
def get_down_block(
    ...
    if down_block_type == "DownBlock2D":
        return DownBlock2D(
            ...
            # temb_channels=temb_channels, ## drop
...
class UNetMidBlock2D(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = self.resnets[0](hidden_states) ## drop temb
...
class CrossAttnDownBlock2D(nn.Module):
    def __init__(
        ...
        add_downsample=True,
        dual_cross_attention=False,
        use_linear_projection=False, ## False
        only_cross_attention=False,
        upcast_attention=False,
    ):
        super().__init__()
...
class AttnSkipDownBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, skip_sample=None):
        output_states = ()
        ...
        if self.downsamplers is not None:
            hidden_states = self.resnet_down(hidden_states)
            for downsampler in self.downsamplers:
...
class AttnSkipUpBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, skip_sample=None):
        for resnet in self.resnets:
            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
        ...
        if self.resnet_up is not None:
            ...
            hidden_states = self.resnet_up(hidden_states)
