...
class FusionNet(nn.Module):
    ...
    def forward(self, image_vis,image_ir):
        ...
        return torch.cat((x_vis_p2, x_inf_p2), dim=1), x_vis_p2, x_inf_p2

def unit_test():
    import numpy as np
    x_ir = torch.tensor(np.random.rand(1,1,256,256).astype(np.float32))
    x_vi = torch.tensor(np.random.rand(1,3,256,256).astype(np.float32))
